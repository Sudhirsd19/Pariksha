...
class StrictChecklistEngine:
    ...
    def evaluate(self, symbol: str, df: pd.DataFrame, 
        ...
        if is_nifty_bullish:
            buy_score += 10
            buy_breakdown["Macro Trend"] += 10
        else:
            sell_score += 10
            sell_breakdown["Macro Trend"] += 10
            
        if is_sector_green:
            buy_score += 5
            buy_breakdown["Macro Trend"] += 5
        else:
            sell_score += 5
            sell_breakdown["Macro Trend"] += 5
        ...
        if promoter_holding >= 50.0:
            buy_score += 5
            buy_breakdown["Fundamentals"] += 5
            sell_score += 5
            sell_breakdown["Fundamentals"] += 5
        ...
        if 50 < rsi < 70:
            buy_score += 10
            buy_breakdown["Momentum"] += 10
        elif rsi > 75:
            buy_reasons.append("RSI Overbought (>75)")
            
        if 30 < rsi < 50:
            sell_score += 10
            sell_breakdown["Momentum"] += 10
        elif rsi < 25:
        ...
        if buy_risk > 0 and (buy_risk / current['close']) < 0.02: 
            buy_score += 10
            buy_breakdown["Risk & Execution"] += 10
        if sell_risk > 0 and (sell_risk / current['close']) < 0.02:
            sell_score += 10
            sell_breakdown["Risk & Execution"] += 10
        ...
        try:
            ...
            if 930 <= time_val <= 1430:
                buy_score += 5
                buy_breakdown["Risk & Execution"] += 5
                sell_score += 5
                sell_breakdown["Risk & Execution"] += 5
        except:
