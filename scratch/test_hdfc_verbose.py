...
def run_backtest_verbose(df, engine_class, name, tp_mult=5.0):
    ...
    for i, s in enumerate(signals):
        print(f"    Signal #{i+1}: {s['type']} at index {s['index']} | price ~{df.iloc[s['index']]['close']:.2f}")
    ...
    if backtest.trades:
        ...
        for i, t in enumerate(backtest.trades):
            pnl_icon = "✅" if t['net_pnl'] > 0 else ("⚪" if abs(t['net_pnl']) < 5 else "❌")
            print(f"  Trade #{i+1}: {t['type']} | Entry: {t['entry_price']:.2f} -> Exit: {t['exit_price']:.2f} | "
                  f"SL: {t['sl']:.2f} | TP: {t['tp']:.2f} | "
                  f"PnL: Rs.{t['net_pnl']:.0f} {pnl_icon} | Reason: {t['exit_reason']}")
